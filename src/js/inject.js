...
this.Notification = function (title, options) {
    // we currently have to manually access the option properties
    // due to a bug in Qt 5.5
    // see: https://bugreports.qt.io/browse/QTBUG-48198
    if (!window.foo)
    {
    new QWebChannel(qt.webChannelTransport, function(channel)
    {
        this.window.foo = channel.objects.foo;
        this.window.foo.showNotification(title, options.body, options.tag, options.icon);
    });
    }
    else
    {
        // https://bugreports.qt.io/browse/QTBUG-48198
        window.foo.showNotification(title, options.body, options.tag, options.icon);
    }
};
